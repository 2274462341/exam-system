    ...
    <div class="looktest">
        <Card title="查看试卷">
            <White>
                <p>
                    <span>课程类型：</span>
                    <span @click="all" ref="all">All</span>
                    <span class="spanList" ref="spanList" v-for="(item, index) in subjectList" :key="index" @click="getChangeBg(item.subject_id)" :class="item.subject_id==subject_id?'active':''">{{item.subject_text}}</span>
                </p>
                <div class="dropdwons">
